...
def process_data(data: List[Dict], tag_dict: Dict) -> List[Tuple[str, str, str]]:
    """
    ...
    for item in data:
        ...
        tag_names = [tag_dict[tag[0]] for tag in tags if tag_dict.get(tag[0], '')]


        processed_data.append((name, status, player_numbers, '|'.join(tag_names)))
...
def main():
    ...
    tag_list= global_users_info_dict.get('game_tags')

    tag_dict = { tag['id']: tag['name'] for tag in tag_list if tag['cat'] == 'Mechanism' }
    print(f'{tag_dict=}')
